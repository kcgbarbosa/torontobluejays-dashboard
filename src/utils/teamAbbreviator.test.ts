import { teamAbbreviator } from './teamAbbreviator';

describe('team name abbreviations', () => {
  it.for([
  ...
  ])(`abbreviate team name to 3 three letters`, ({ name, abbreviation }) => {
    expect(teamAbbreviator(name)).toBe(abbreviation);
  });
});
